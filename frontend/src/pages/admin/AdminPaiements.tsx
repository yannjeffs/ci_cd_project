...
useEffect(() => {
  ...
  const handleValider = async (id: number) => {
    if (
      !confirm(
        "Confirmer la validation de ce paiement ?\n\nCela créera automatiquement l'enrôlement de l'étudiant et lui enverra une notification.",
      )
    )
      return;
    setProcessing(id);
  ...
  const handleRejeter = async (id: number) => {
    const motif = prompt(
      "Motif du rejet (obligatoire pour informer l'étudiant):",
    );
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="REJETE">
                ✗ Rejetés (
                {paiements.filter((p) => p.statut === "REJETE").length})
              </option>
            </select>
            <svg
        ...
        {filteredPaiements.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-slate-400 text-sm mt-2">
              Changez les filtres pour voir plus de résultats
            </p>
          </div>
        ) : (
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-linear-to-br from-blue-400 to-indigo-500 rounded-xl flex items-center justify-center text-white font-bold shadow-sm">
                          {p.etudiant?.user?.prenom?.charAt(0)}
                          {p.etudiant?.user?.nom?.charAt(0)}
                        </div>
                        <div>
...
function StatCard({
  ...
  return (
    <div
      className={`${bgColor} rounded-2xl p-6 border-2 ${borderColor} shadow-sm hover:shadow-md transition-all duration-300 group`}
    >
      <div className="flex items-center justify-between mb-4">
        <div
          className={`w-14 h-14 bg-linear-to-br ${gradient} rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform`}
        >
          <span className="text-3xl">{icon}</span>
        </div>
        <div
          className={`text-4xl font-black bg-linear-to-br ${gradient} bg-clip-text text-transparent`}
        >
          {value}
        </div>
      </div>
